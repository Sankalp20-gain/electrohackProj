...
export default function HostelDashboard() {
  ...
  const handleCreateOrder = async () => {
    ...
    setOrderName("");
    setTimeLimit("");
    alert("✅ Your group order room has been successfully created!");
    ...
    <div className={`${darkMode ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"} min-h-screen p-6`}>
      ...
      <div className={`flex items-center justify-between mb-6 ${darkMode ? "bg-gray-800" : "bg-white"} bg-opacity-90 backdrop-blur-md p-4 rounded-xl shadow-md`}>
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => setDarkMode(!darkMode)} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 font-medium transition">
      ...
      {/* Create New Group Order Button */}
      <div className="mb-6 flex justify-end">
        <button onClick={() => setShowModal(true)} className="px-6 py-3 bg-yellow-400 hover:bg-yellow-500 text-white font-semibold rounded-xl shadow-md transition">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {orders
          ...
          .map((order) => {
            ...
            return (
              <div key={order.id} className={`${darkMode ? "bg-gray-800" : "bg-white"} shadow-md rounded-2xl p-6 hover:shadow-xl transition transform hover:scale-105`}>
                ...
                <p className={`mb-1 font-medium ${remainingSeconds / 60 <= 5 ? "text-red-600" : "text-gray-700"}`}>
                  time remain : <CountdownTimer initialSeconds={remainingSeconds} />
                </p>
                <p className={`mb-2 font-medium ${order.status === "Completed" ? "text-green-600" : "text-yellow-600"}`}>
                  Status: {order.status}
                </p>
                <Link to={`/dashboard/${hostelId}/order/${order.id}`} className="mt-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium transition">
                  View Details
                </Link>
                <button
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className={`${darkMode ? "bg-gray-800 text-white" : "bg-white text-black"} p-6 rounded-xl shadow-lg w-96`}>
            ...
            <input type="text" placeholder="Enter Order Name" value={orderName} onChange={(e) => setOrderName(e.target.value)} className="w-full px-3 py-2 border rounded-lg mb-3 text-black" />
            <input type="number" placeholder="Enter Time Limit (minutes)" value={timeLimit} onChange={(e) => setTimeLimit(e.target.value)} className="w-full px-3 py-2 border rounded-lg mb-3 text-black" />
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 bg-gray-400 hover:bg-gray-500 text-white rounded-lg">Cancel</button>
              <button onClick={handleCreateOrder} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg">Create</button>
            </div>
          </div>
        </div>
      )}
